...
function writeFragment(fragment) {
  ...
  try {
    ...
    throw new Error('unable to write fragment metadata to DynamoDB');
  }
}
...
async function readFragment(ownerId, id) {
  ...
  try {
    ...
    throw new Error('unable to read fragment metadata from DynamoDB');
  }
}
...
async function writeFragmentData(ownerId, id, data) {
  ...
  try {
    ...
    throw new Error('unable to upload fragment data to S3');
  }
}
...
async function readFragmentData(ownerId, id) {
  ...
  try {
    ...
    throw new Error('unable to read fragment data from S3');
  }
}
...
async function listFragments(ownerId, expand = false) {
  ...
  try {
    ...
    logger.error({ err, params }, 'error getting all fragments metadata for user from DynamoDB');
    throw new Error('unable to get all fragment metadata from DynamoDB');
  }
}
...
async function deleteFragment(ownerId, id) {
  // Create the DELETE API params from our details: S3
  const s3Params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    // Our key will be a mix of the ownerID and fragment id, written as a path
    Key: `${ownerId}/${id}`,
  ...
  try {
    ...
    logger.info('Success - fragment data deleted');
  } catch (err) {
  ...
  // Create the DELETE API params from out details: DynamoDB
  ...
  try {
    ...
    logger.info('Success - fragment metadata deleted');
  } catch (err) {
